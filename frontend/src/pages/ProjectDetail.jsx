...
export default function ProjectDetail() {
    ...
    const loadResources = async () => {
        try {
            const [reposRes, aiRes] = await Promise.all([
                repositories.getAll(),
                aiResources.getAll(),
            ]);
                        ...
                        {project.languages?.length === 0 ? (
                            ...
                            <div className="flex flex-wrap gap-2">
                                {project.languages?.map((pl) => (
                                    <div
                                        key={pl.id}
                                        className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-900 dark:text-white"
                                    >
                                        {pl.language.name}
                                    </div>
                                ))}
